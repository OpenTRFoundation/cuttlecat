...
function buildConfigFromEnvVars() {
    return cleanEnv(process.env, {
        LOCATIONS_MASTER_FILE: str({
            desc: "Path to the master locations file.",
        }),
        LOCATIONS_ADDITIONAL_FILE: str({
            desc: "Path to the additional locations file.",
        }),
        LOCATIONS_EXCLUDE_FILE: str({
...
export function main() {
    ...
    const locationsMasterList:LocationsMaster = JSON.parse(readFileSync(config.LOCATIONS_MASTER_FILE, "utf8"));
    const locationsAdditionalList:LocationsAdditional = JSON.parse(readFileSync(config.LOCATIONS_ADDITIONAL_FILE, "utf8"));
    const locationsExcludeList:LocationsExclude = JSON.parse(readFileSync(config.LOCATIONS_EXCLUDE_FILE, "utf8"));
